...
- IP holders can set default values for revenue splitting. Those values can be based on the influence of the IP, the revenue made by the remixed content etc. For example, an IP holder can decide to cash-out for any remixed content whose IP attribution is more than 2%, or to leave 50% of their revenue to the IP remixer if they made less than 10k$/month.
